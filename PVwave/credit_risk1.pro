...
function HWSwapOption_risk,EXPIRATION,crbtree,spread=spread
r=crbtree.arbre.r
nodes=crbtree.arbre.nodes
k=crbtree.arbre.k
p1=crbtree.arbre.p1
...
for i=imax-1,0,-1 do begin             ;index pointant dans virtual_taux_crb(i,*,*)
	for j=0,2*m-1 do begin         ; index d etat
		if nodes(i,j) eq 1 then begin
			...
			if i eq imax-1 then  opt(i,j)=support(i,j) else  $
			opt(i,j)=exp(-r(i,j)*delta_t)*(p1(i,j)*opt(i+1,k(i,j)+1)+p2(i,j)*opt(i+1,k(i,j))+p3(i,j)*opt(i+1,k(i,j)-1))

			if i ge irisk  then begin
...
; fonction qui calcule en tout point d un arbre une valeur de bond sur la courbe zero coupons correspondant au neud
; et calcule la valeur de l option qui represente la perte du bond en cas de default de l emetteur
...
function HWbondOption_risk ,notionnel,maturity,periodpaiements,fixedrate,crbtree
r=crbtree.arbre.r
nodes=crbtree.arbre.nodes
k=crbtree.arbre.k
p1=crbtree.arbre.p1
...
for i=imax-1,0,-1 do begin             ;index pointant dans virtual_taux_crb(i,*,*)
	for j=0,2*m-1 do begin         ; index d etat
		if nodes(i,j) eq 1 then begin
			...
			if i eq imax-1 then opt(i,j)=support(i,j) else $
				if i ge irisk then opt(i,j)=Max([exp(-r(i,j)*delta_t)*($
							p1(i,j)*opt(i+1,k(i,j)+1)+$
							p2(i,j)*opt(i+1,k(i,j))+$
							p3(i,j)*opt(i+1,k(i,j)-1)) , support(i,j)]) $
				else opt(i,j)=exp(-r(i,j)*delta_t)*($
							p1(i,j)*opt(i+1,k(i,j)+1)+$
							p2(i,j)*opt(i+1,k(i,j))+$
							p3(i,j)*opt(i+1,k(i,j)-1))
...
function HWSwapImp_spread,EXPIRATION,curveZ,crbtree,prob_default,x1=x1,x2=x2,xacc=xacc,jmax=jmax,k=k
...
if keyword_set(jmax) eq 0. then jmax=40
fmid=double(EVAL_SWAP(curveZ,0.,x2,k=k)-prime)
f=double(EVAL_SWAP(curveZ,0.,x1,k=k)-prime)
id=0
while f*fmid gt 0. and id lt 7 do begin
	x2=2*x2
fmid=EVAL_SWAP(curveZ,0.,x2,k=k)-prime
	id=id+1
endwhile
;print,'fmid=',fmid,' /f=',f
if f*fmid ge 0 then begin
...
fmid=EVAL_SWAP(curveZ,0.,xmid,k=k)-prime
	if fmid le 0 then rtbis=xmid
;print,rtbis,rtbis+dx,EVAL_SWAP(curveZ,0.,rtbis,k=k),EVAL_SWAP(curveZ,0.,rtbis+dx,k=k),'but=',prime
	if abs(dx) lt xacc or fmid eq 0 then return,rtbis
endwhile
	if abs(dx) lt xacc or fmid eq 0 then return,rtbis else print,'nb max d iteration atteint:',jmax
return,0
...
;********** C A L C U L    D E    C A P S   E T   A U T R E S   S W A P S    C O N D I T O N N E L S*********



; cas du cap 
function EVAL_SWAP_PAYBACK,curveZ,t
terme=0.5
periodefinancement=0.5
Notionnel=100.
k=0.08
r=interpolation(curveZ,terme)
return,Max([exp(r*periodefinancement)-exp(k*periodefinancement),0.])*Notionnel
end

function HWCondSwap,PaymentDates,crbtree
r=crbtree.arbre.r
nodes=crbtree.arbre.nodes
k=crbtree.arbre.k
p1=crbtree.arbre.p1
...
delta_r=crbtree.arbre.delta_r
n=crbtree.arbre.nt
npd=n_elements(PaymentDates)
m=crbtree.arbre.nr
virtual_discount_crb=crbtree.discount_tree
EXPIRATION=paymentdates(npd-1)
imax=int(EXPIRATION/delta_t)+1         ;index temporel (date=imax*delta_t)
support=fltarr(imax,2*m)
opt=fltarr(imax,2*m)
for i=imax-1,0,-1 do begin             ;index pointant dans virtual_taux_crb(i,*,*)
	for j=0,2*m-1 do begin         ; index d etat
		if nodes(i,j) eq 1 then begin
			...
			for km=0,maturity_max-1 do curveZ(km,1)=virtual_discount_crb(i,km,j)^(-1./curveZ(km,0))-1. 
								  ;index pointant dans virtual_taux_crb(i,km,*)
      			for ip=0,npd-1 do if paymentdates(ip) ge i*delta_t and  paymentdates(ip) lt (i+1.)*delta_t then begin
				support(i,j)=support(i,j)+EVAL_SWAP_PAYBACK(curveZ, paymentdates(ip))*exp(-r(i,j)*(paymentdates(ip)-i*delta_t))
				end
			if i eq imax-1 then opt(i,j)=support(i,j)  else  begin
				  opt(i,j)=exp(-r(i,j)*delta_t)*(p1(i,j)*opt(i+1,k(i,j)+1)+p2(i,j)*opt(i+1,k(i,j))+p3(i,j)*opt(i+1,k(i,j)-1))
				  opt(i,j)=opt(i,j)+support(i,j)
			endelse
...
sigma=0.015
n=15
paymentdates=[1.,1.5,2.,2.5,3.]
crbtree=HWbuild_crbtree(curverz,a,sigma,n)
print,HWCondSwap(PaymentDates,crbtree)
end
...
function EVAL_SWAP,curvezero,t,spread,k=k
maturite=8
tf=1.
curvez=curvezero
curvef=taux_forward(curvezero,tf)
...
curvek=replicate(0.12 ,maturite)  ;taux fixe
if keyword_set(k) ne 0 then curvek=replicate(k ,maturite)  ;taux fixe
...
spread=fltarr(n_elements(prob))
fixedrate=0.03
for i=0,n_elements(prob)-1 do spread(i)=HWSwapImp_spread(EXPIRATION,curvezero,crbtree,prob(i),k=0.1)
plot,prob,spread
end


;courbe des probabilte des spread implicit(probabilite de default) quand on fait varier k
pro testswap04
curvezero=transpose([[1.,0.1],[2.,0.105],[3.,0.11],[4.,0.1125],[5,0.115],[6.,0.117],[7.,0.1185],[8.,0.1195],[9.,0.1205],[10.,0.121],[11.,0.1205]])
EXPIRATION=8.
a=0.1
...
EXPIRATION=5.
prob=0.001
k=[0.05,0.08,0.1,0.12,0.15,0.2]
spread=fltarr(n_elements(k))
for i=0,n_elements(k)-1 do spread(i)=HWSwapImp_spread(EXPIRATION,curvezero,crbtree,prob,k=k(i))
plot,k,spread
end


; decomposition du risk sur le temps pour un swap

;   STRUCTURE DU SWAP

function EVAL_SWAP1,curvezero,t,spread
maturite=6
tf=1.
curvez=curvezero
curvef=taux_forward(curvezero,tf)
paymntdate=findgen(maturite+2)+tf
curveN=replicate(1000.,maturite+2)
curveN(maturite)=0.
curveN(Maturite+1)=0.
curvek=replicate(0.1 ,maturite+2)  ;taux fixe
...
;test de HWSwapOption
pro test04a
curverz=transpose([[1.,0.1],[2.,0.105],[3.,0.11],[4.,0.1125],[5,0.115],[6.,0.117],[7.,0.1185],[8.,0.1195],[9.,0.1205],[10.,0.121],[11.,0.1205]])
EXPIRATION=6.
a=0.1
sigma=0.015
n=22
crbarbre=HWbuild_crbtree(curverz,a,sigma,n)
x=HWSwapOption_risk(EXPIRATION,crbarbre)
y=HWSwapOption(EXPIRATION,crbarbre)
print,y-x
plot,y-x
end

; decomposition du risk sur le temps pour un bond
pro test04b
curverz=transpose([[1.,0.1],[2.,0.105],[3.,0.11],[4.,0.1125],[5,0.115],[6.,0.117],[7.,0.1185],[8.,0.1195],[9.,0.1205],[10.,0.121],[11.,0.1205]])
a=0.1
sigma=0.015
...
crbtree=HWbuild_crbtree(curverz,a,sigma,n)
print,'prix HW             ,Prix actuariel
y=bond(notionnel,maturity,periodpaiements,fixedrate,curverz,0)
print,'prix du bond :',y
x=HWBondOption_risk(notionnel,maturity,periodpaiements,fixedrate,crbtree)
print,y-x
plot,y-x
end
